module.exports.run = async (bot, message, args) => { 
    ...
    if (!args[0]) {
        let embed = new Discord.RichEmbed()
            .setAuthor(`${bot.user.username} v${config.version} command list`, bot.user.avatarURL, "https://github.com/FriendsNone/FunctionKey-Bot")
        ...
        let embed = new Discord.RichEmbed()
            ...
            .setColor("BLUE")
            .setFooter(`Parameters encased in parentheses are optional | ${bot.user.username} v${config.version}`)
        message.channel.send({ embed });
    } catch (err) {
